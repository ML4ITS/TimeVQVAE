...
def build_data_pipeline(batch_size, dataset_importer: DatasetImporterUCR, config: dict, kind: str) -> DataLoader:
    ...
    if kind == 'train':
        ...
        return DataLoader(train_dataset, batch_size, num_workers=num_workers, shuffle=True, drop_last=False)  # `drop_last=False` due to some datasets with a very small dataset size.
    elif kind == 'test':
